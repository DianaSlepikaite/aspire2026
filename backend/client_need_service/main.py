"""
FastAPI application entry point for Client Need Service Agent.
"""
...
from client_need_service.core.logging import setup_logging
from client_need_service.core.database import initialize_database, close_database
from client_need_service.core.exceptions import ServiceError
from client_need_service.api.v1.router import api_router
...
async def lifespan(app: FastAPI):
    ...
    """
    # Startup
    logger.info("Starting Client Need Service Agent...")
    ...
    # Shutdown
    logger.info("Shutting down Client Need Service Agent...")
...
app = FastAPI(
    title="Client Need Service Agent",
    description="AI-powered service for extracting and understanding client project needs through natural conversation",
    version="1.0.0",
    lifespan=lifespan,
    docs_url="/docs",
    redoc_url="/redoc",
...
async def root():
    ...
    return {
        "service": "Client Need Service Agent",
        "version": "1.0.0",
        "status": "running",
        "docs": "/docs",
